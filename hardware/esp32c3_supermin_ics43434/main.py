...
# ------------------------
# CONFIG SOUND — ICS-43434 via I2S (digital MEMS mic)
# ------------------------
# ICS-43434 is an I2S slave: ESP32-C3 is master and generates the clocks.
# L/R → GND selects the LEFT channel; we read STEREO frames and take the
# left slot (every other 32-bit word).
audio_in = I2S(
    0,
    sck=Pin(config.MIC_SCK_GPIO),   # BCLK → GPIO5
    ws=Pin(config.MIC_WS_GPIO),     # LRCK → GPIO4
    sd=Pin(config.MIC_SD_GPIO),     # DATA → GPIO10
    mode=I2S.RX,
...
# Byte offset of the mic's slot inside each 8-byte stereo frame.
# L/R → 3V3 = RIGHT channel (offset 4); L/R → GND = LEFT channel (offset 0).
MIC_SLOT_OFF = 4 if config.MIC_SLOT == "R" else 0
...
async def get_calibration():
    """Measure the ambient noise floor for reference."""
    print("Calibrating ...")
    buf = bytearray(config.MIC_BUF_BYTES)
    n = audio_in.readinto(buf)
    # 24-bit data is left-justified in the 32-bit slot → word >> 8
    min_v = 0
    max_v = 0
    for i in range(0, n - 3, 8):  # mic slot only
        v = struct.unpack_from("<i", buf, i + MIC_SLOT_OFF)[0] >> 8
        if v < min_v: min_v = v
        if v > max_v: max_v = v
    baseline = max_v - min_v
    print("Baseline amplitude:", baseline)
    return baseline
...
async def sound_monitor(char, connection):
    ...
    while True:
        # Read one 50ms I2S window (blocks until the DMA fills the buffer)
        n = audio_in.readinto(buf)
        nframes = n // 8  # STEREO: left + right 32-bit words per frame

        # Peak-to-peak amplitude on the mic's slot (L/R pin selects it)
        max_v = -0x80000000
        min_v = 0x7FFFFFFF
        for i in range(nframes):
            v = struct.unpack_from("<i", buf, i * 8 + MIC_SLOT_OFF)[0] >> 8  # 24-bit data
            if v > max_v: max_v = v
            if v < min_v: min_v = v
...
async def main():
    ...
    while True: #to be able to reconnect with the device
