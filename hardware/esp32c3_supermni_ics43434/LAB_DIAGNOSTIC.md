...
SD pin is electrically **floating** — internal pull resistors flip the reading (PULL_DOWN→zeros, PULL_UP→`-1`), so the mic's push-pull SD output is not reaching the GPIO. Most likely: intermittent/broken SD jumper contact, missing VDD to the mic, or missing ≥1 µF decoupling cap on VDD (required by the ICS-43434 datasheet; without it the mic fails to start).

## Checklist (power off first)
1. Multimeter at the **mic's VDD pin** vs GND → must read ~3.3 V.
2. Add ≥1 µF capacitor between VDD and GND, as close to the mic as possible.
3. Continuity check: SD↔GPIO10, SCK↔GPIO5, WS↔GPIO4, VDD→3V3, GND→GND, L/R→3V3.
4. Power on, run `mpremote connect <port> run probe_i2s.py` (in this folder).
5. Pass = amplitude > 0 in capture step AND pull test doesn't change the reading. Then flash `config.py` + `main.py` and calibrate thresholds.
